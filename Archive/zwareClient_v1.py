...
class zwareClientClass:
	...
	#@}
	def poll_notification_sensor(self):
		if self.deviceDictionaryList[self.NOTIFICATIONINTERFACE]['foundDevice'] == 1:
			...
			if r == None:
				...
				sensor_event = int(xml[sub1+7])	#find current state of PIR sensor, 0 = no motion, 8 = motion detected
			...
			if self.deviceDictionaryList[self.NOTIFICATIONINTERFACE]['previouslyFoundDevice'] == 0:
				...
				self.deviceDictionaryList[self.NOTIFICATIONINTERFACE]['defaultState'] = sensor_event
				#aikhong
				topic = TOPIC_PUBLISH + '/6/' +'event'
				self.mqttpublish(topic, sensor_event)	# 8 - motion, 0 is idle
				#aikhong
			if self.deviceDictionaryList[self.NOTIFICATIONINTERFACE]['defaultState'] != sensor_event:
				self.debugData.insert(INSERT,"Polling: Notificaton Device Status Updated\n")
				#self.debugData.insert(INSERT,notification_device_data)	#debugging
				self.debugData.insert(INSERT,sensor_event)
				self.deviceDictionaryList[self.NOTIFICATIONINTERFACE]['defaultState'] = sensor_event
				#aikhong
				topic = TOPIC_PUBLISH + '/6/' + 'event'
				self.mqttpublish(topic, sensor_event)	# 8 - motion, 0 is idle
