...
#aikhong
#mqtt topic settings
TOPIC_OF_SUBSCRIBE = "treadmill/sub"
TOPIC_PUBLISH  = "sgoffice"
#TOPIC_PUBLISH_DLIST = "sgoffice/devicelist"
			...
			for ep in range(len(eps)):
				...
				for intf in range(len(intfs)):
					if (intfs[intf].get('name') != "Unknown"):
						...
						topic = TOPIC_PUBLISH + "/" + nodeid +"/" + "Type of Device" + "/"
...
#aikhong
					...
					for intf in range(len(intfs)):
						if (intfs[intf].get('name') != "Unknown"):
							...
							if commandclass == "COMMAND_CLASS_ALARM":
								...
								poll = "binary_switch"
								#temp measure only - aikhong - to be remove and replace with mqtt
								self.deviceDictionaryList[self.BINARYSWITCHINTERFACE]['foundDevice'] = 1
	...
	def poll_binary_switch(self, node):
		...
		if self.deviceDictionaryList[self.BINARYSWITCHINTERFACE]['foundDevice'] == 1:
			...
			self.enable_disable_binary_switch()	# to be remove
			#print("\nValue of node in poll:" +str(node))
		...
		if self.deviceDictionaryList[self.BINARYSWITCHINTERFACE]['foundDevice'] == 0:
			...
			if( value == 255):
				...
				if v == 255:
					...
					binary_event[node-1][0] = v			#update the new device status
				else:
					self.debugData.insert(INSERT,"Switch could not be turned on\n")
					binary_event[node-1][0] = 0
				...
				if v == 0:
					...
					binary_event[node-1][0] = value		#update the new device status
				else:
	...
	def poll_notification_sensor(self, node):
		...
		if self.deviceDictionaryList[self.NOTIFICATIONINTERFACE]['foundDevice'] == 1:
			...
			if r == None:
				...
				##self.debugData.insert(INSERT,"device value: " +device+ '\n')	#debugging
				...
				if last_sensor_event == new_sensor_event:
					self.debugData.insert(INSERT,"No change in state for the device\n")
				else:
					topic = TOPIC_PUBLISH + '/' + loc + '/' + 'event'
					self.mqttpublish(topic, new_sensor_event)	# 8 - motion, 0 is idle

				#self.deviceDictionaryList[self.NOTIFICATIONINTERFACE]['ZWnode'] = loc



			# comment out first
			#if self.deviceDictionaryList[self.NOTIFICATIONINTERFACE]['previouslyFoundDevice'] == 0:
			#	self.debugData.insert(INSERT,"Polling: Notificaton Device found in network\n")
			#	self.deviceDictionaryList[self.NOTIFICATIONINTERFACE]['previouslyFoundDevice'] = 1
			#	#self.debugData.insert(INSERT,notification_device_data)	#debugging
			#	self.deviceDictionaryList[self.NOTIFICATIONINTERFACE]['defaultState'] = sensor_event[node]
				#aikhong
			#	topic = TOPIC_PUBLISH + '/' + loc + '/' + 'event'
			#	self.mqttpublish(topic, sensor_event[node])	# 8 - motion, 0 is idle
				#aikhong
			#if self.deviceDictionaryList[self.NOTIFICATIONINTERFACE]['defaultState'] != sensor_event[node]:
			#	self.debugData.insert(INSERT,"Polling: Notificaton Device Status Updated\n")
				#self.debugData.insert(INSERT,notification_device_data)	#debugging
			#	self.debugData.insert(INSERT,sensor_event[node])
			#	self.deviceDictionaryList[self.NOTIFICATIONINTERFACE]['defaultState'] = sensor_event[node]
				#aikhong
			#	topic = TOPIC_PUBLISH + '/' + loc + '/' + 'event'
			#	self.mqttpublish(topic, sensor_event[node])	# 8 - motion, 0 is idle
...
#mqtt code
	...
	def poll_server(self):
		while self.runPollingThread:
			...
			#self.poll_multilevel_sensor()
			#aikhong
			#self.poll_notification_sensor()
			#self.debugData.insert(INSERT,Znodeid)
			#aikhong
			time.sleep(2)
